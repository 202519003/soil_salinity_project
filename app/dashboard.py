...
DARK = {
    ...
    "sidebar": "#0f2040", "input_bg": "#0f2040", "table_bg": "#0f2040",
...
LIGHT = {
    ...
    "sidebar": "#e8f0f8", "input_bg": "#ffffff", "table_bg": "#ffffff",
...
SMART_POLICY = {
    ("coastal","Critical"): "🚨 URGENT — Emergency coastal bund construction + mangrove belt restoration. Ban new borewells within 5 km of coast. Immediate gypsum application.",
    ("coastal","High"):     "🔴 HIGH — Install subsurface drainage + coastal windbreaks. Monitor groundwater EC monthly. Restrict irrigation during high-tide periods.",
    ("coastal","Moderate"): "⚠️ MONITOR — Seasonal ECe soil testing. Limit irrigation near tidal zones. Encourage salt-tolerant crop varieties.",
    ("coastal","Low"):      "✅ STABLE — Annual coastal soil health check. Maintain existing bund structures. No immediate intervention needed.",
    ("canal","Critical"):   "🚨 URGENT — Immediately stop canal supply to affected fields. Apply gypsum + leaching with good-quality water. Install subsurface tile drains.",
    ("canal","High"):       "🔴 HIGH — Reduce canal water allocation by 30%. Install field-level drainage. Switch to drip/sprinkler irrigation to reduce waterlogging.",
    ("canal","Moderate"):   "⚠️ MONITOR — Improve irrigation scheduling to avoid over-watering. Quarterly ECe monitoring. Check drainage outlets for blockage.",
    ("canal","Low"):        "✅ STABLE — Maintain current irrigation practices. Annual soil health card. Monitor canal water quality for dissolved salts.",
    ("inland","Critical"):  "🚨 URGENT — Ban new borewell drilling immediately. Bio-drainage plantations (Eucalyptus/Prosopis). Gypsum + deep ploughing + leaching.",
    ("inland","High"):      "🔴 HIGH — Strictly restrict groundwater extraction depth. Switch to micro-irrigation. Apply gypsum and organic matter to improve soil structure.",
    ("inland","Moderate"):  "⚠️ MONITOR — Monitor groundwater depth and EC quarterly. Reduce irrigation frequency. Promote mulching to reduce surface evaporation.",
    ("inland","Low"):       "✅ STABLE — Annual groundwater quality check. Maintain current practices. Promote rainwater harvesting to reduce borewell dependency.",
    ("hilly","Critical"):   "⚠️ INVESTIGATE — Unusual for hilly zone. Likely industrial effluent or upstream contamination. Conduct soil and water quality investigation immediately.",
    ("hilly","High"):       "⚠️ INVESTIGATE — Check for upstream land-use change or irrigation source contamination. Soil sampling recommended across multiple depths.",
    ("hilly","Moderate"):   "✅ MONITOR — Likely temporary weather-driven effect. Monitor for 2 seasons. Natural terrain drainage should self-correct.",
    ("hilly","Low"):        "✅ STABLE — Natural drainage active. No intervention needed. Maintain forest cover to preserve drainage capacity.",
}

DEFAULT_METRICS = pd.DataFrame({
    "model":["Linear Regression","Random Forest","XGBoost","TFT Transformer"],
    "mae":  [7.50,7.48,7.58,5.91],
    "rmse": [8.71,8.80,9.57,8.31],
    "r2":   [0.737,0.732,0.683,0.757],
    "mape": [np.nan,np.nan,np.nan,10.6]
})
...
def sc(cls): 
    return CLR.get(cls, T["accent"])

def ax_style(ax, fig):
    fig.patch.set_facecolor(T["chart_fig"])
    ax.set_facecolor(T["chart_bg"])
    ax.tick_params(colors=T["tick"], labelsize=8)
    ax.xaxis.label.set_color(T["text_sec"])
    ax.yaxis.label.set_color(T["text_sec"])
    ax.title.set_color(T["text_pri"])
    for sp in ax.spines.values(): 
        sp.set_edgecolor(T["border"])
    ax.grid(alpha=0.18, color=T["grid"], linewidth=0.5)

def compute_sspi_class(sspi):
    if   sspi >= 75: return "Critical"
    elif sspi >= 50: return "High"
    elif sspi >= 25: return "Moderate"
    else:            return "Low"

def find_database():
    """Try multiple possible database locations"""
    possible_paths = [
        "data/processed/salinity_db.sqlite",
        "../data/processed/salinity_db.sqlite",
        "../../data/processed/salinity_db.sqlite",
        Path(__file__).parent.parent / "data" / "processed" / "salinity_db.sqlite",
    ]
...
@st.cache_data
def load_data():
    """Load data from SQLite database with better error handling"""
    ...
    if db_path is None:
        ...
        The file `data/processed/salinity_db.sqlite` is missing from your repository.
        
        **To fix this:**
        1. Make sure you've uploaded the database file to GitHub
        2. The file should be at: `data/processed/salinity_db.sqlite`
        3. Or run your data processing scripts to generate it
        ...
        **Repository structure should be:**
        ...
        ├── data/
        │   └── processed/
        │       └── salinity_db.sqlite  ← THIS FILE IS REQUIRED
        ├── outputs/
        └── scripts/
        ```
        """)
    ...
    try:
        ...
        try:
            ...
            if len(m) == 0 or "r2" not in m.columns:
        ...
        # Validate data
        if len(h) == 0:
            st.error("❌ Database is empty! No historical data found.")
            st.stop()
        
        # Recompute sspi_class from score
        h["sspi_class"] = h["sspi"].apply(compute_sspi_class)
        
        # Overwrite zone_type from ZONE_MASTER
        ...
        if "district" in f.columns:
            ...
            f["zone_type"] = f.get("zone_type", "inland")
        ...
        Database path tried: `{db_path}`
        
        Please check that:
        1. The database file is properly formatted
        2. Required tables exist: sspi_history, sspi_forecast, model_metrics
...
div[data-baseweb="radio"] label {{ color: {T["text_pri"]} !important; font-size: .8rem !important; }}
...
.ic.Critical {{ border-color: {CLR["Critical"]}; }} .ic.High {{ border-color: {CLR["High"]}; }} .ic.Moderate {{ border-color: {CLR["Moderate"]}; }} .ic.Low {{ border-color: {CLR["Low"]}; }}
...
.cls-critical {{ color: {CLR["Critical"]}; font-weight: 700; }}
.cls-high {{ color: {CLR["High"]}; font-weight: 700; }}
.cls-moderate {{ color: {CLR["Moderate"]}; font-weight: 700; }}
.cls-low {{ color: {CLR["Low"]}; font-weight: 700; }}
...
# Build latest_all and enforce zone_type from ZONE_MASTER
...
with st.sidebar:
    ...
    for col, (icon, zone) in zip(qz_cols, ZONE_LIST):
        ...
        with col:
            if is_active: 
                st.markdown("<div class='zone-active'>", unsafe_allow_html=True)
            ...
            if clicked:
                ...
                if not zone_dists:
                    st.toast(f"No districts for {zone.title()}", icon="⚠️")
                elif st.session_state.active_zone == zone:
                    st.session_state.active_zone = None
                    st.rerun()
                else:
    ...
    if st.session_state.active_zone:
        ...
        st.markdown(f"""<div style='background:{T["card"]};border:1.5px solid {T["accent"]};border-radius:8px;
          ...
          <span style='font-size:.62rem;color:{T["text_sec"]}'>{n} districts</span></div>""",
          unsafe_allow_html=True)

        cur_sel = st.session_state.sel if st.session_state.sel in zone_dists else zone_dists[0]
        idx_in_zone = zone_dists.index(cur_sel)
        chosen_zone_dist = st.selectbox("Pick district", options=zone_dists, index=idx_in_zone,
        ...
        if st.session_state.sel and st.session_state.sel in all_districts:
    ...
    icons_sb = {
        "Linear Regression": ("📐", "#5c6bc0"),
        "Random Forest": ("🌲", "#43a047"),
        "XGBoost": ("⚡", "#fb8c00"),
        "TFT Transformer": ("🤖", T["accent"])
    }
    for _, row in metrics.iterrows():
        icon, clr_m = icons_sb.get(row["model"], ("📊", T["accent"]))
        acc = round(row["r2"] * 100, 1)
...
latest_yr = int(dh["year"].max()) if len(dh) > 0 else 2025
...
def zb(z): 
    return f"<span class='zb zb-{z}'>{ZONE_ICON.get(z, '')} {z.upper()}</span>"

zone_badge = ""
if st.session_state.active_zone:
    az = st.session_state.active_zone
    n = len([d for d in all_districts if zone_map.get(d) == az])
...
display_district = (f"📍 {st.session_state.sel} &nbsp; {zb(zone_type)}"
...
st.markdown(f"""<div class='app-hdr'>
  <div><div class='app-title'>🌾 Gujarat Soil Salinity Intelligence Platform{zone_badge}</div>
  <div class='app-sub'>Spatio-Temporal Prediction 2015–2030 · ML + Temporal Fusion Transformer · 33 Districts · MODIS GEE · NASA POWER · CSSRI Bharuch</div></div>
...
kpis = [
    ...
    (k3, "🔮", "Forecast 2030",
        f"{sspi_2030:.1f}" if sspi_2030 and st.session_state.sel else "—",
        f"📈 {trend_dir}",
        CLR["Critical"] if (sspi_2030 or 0) >= 75 else CLR["High"] if (sspi_2030 or 0) >= 50 else CLR["Low"]),
    ...
    (k6, "📊", "10yr Change",
        ...
        "SSPI 2015→2025",
        CLR["Critical"] if sspi_delta > 5 else CLR["Low"] if sspi_delta < 0 else T["text_sec"]),
...
with t5_b1:
    if st.button("📊 Historical SSPI", key="btn_hist", use_container_width=True): 
        st.session_state["t5"] = 0
with t5_b2:
    if st.button("🔮 Forecast 2026-2030", key="btn_fore", use_container_width=True): 
        st.session_state["t5"] = 1
with t5_b3:
    if st.button("🗺️ All Districts Latest", key="btn_all", use_container_width=True): 
        st.session_state["t5"] = 2
...
with tab1:
    ...
    with cm:
        st.markdown("<div class='sh'>Interactive Map — click any district</div>", unsafe_allow_html=True)

        # Filter map rows by active zone
        if st.session_state.active_zone:
            map_rows = latest_all[
                latest_all["district"].map(ZONE_MASTER).fillna("inland") == st.session_state.active_zone
            ]
        else:
            map_rows = latest_all
        ...
        for _, row in map_rows.iterrows():
            ...
            if dn not in COORDS: 
                continue
            lat, lon = COORDS[dn]
            isel = (dn == st.session_state.sel)

            dot_class = compute_sspi_class(float(row["sspi"]))
            clr = sc(dot_class)
            zt = ZONE_MASTER.get(dn, "inland")

            folium.CircleMarker(
                location=[lat, lon],
                radius=22 if isel else 14,
                color="white" if isel else clr,
                weight=3 if isel else 1.5,
                fill=True,
                fill_color=clr,
                fill_opacity=1.0 if isel else 0.85,
                tooltip=folium.Tooltip(
                    f"<div style='font-family:sans-serif;font-size:12px;"
                    f"background:#0f2040;color:#e8f0fe;"
                    f"padding:8px 12px;border-radius:8px;border:2px solid {clr};"
                    f"min-width:160px;box-shadow:0 4px 12px rgba(0,0,0,.4)'>"
                    f"<b style='font-size:13px;color:{clr}'>{dn}</b><br>"
                    f"<span style='color:#90afd4;font-size:10px'>SSPI: </span>"
                    f"<b style='color:{clr}'>{row['sspi']:.1f}</b> &nbsp;"
                    f"<b style='color:{clr}'>{dot_class}</b><br>"
                    f"<span style='color:#90afd4;font-size:10px'>Zone: </span>"
                    f"<b style='color:#e8f0fe'>{zt}</b><br>"
                    f"<i style='font-size:9px;color:#90afd4'>Click to select</i></div>",
                    sticky=False
                ),
                ...
                folium.Marker(
                    ...
                    icon=folium.DivIcon(
                        html=(f"<div style='background:{clr};color:#fff;font-size:9px;font-weight:700;"
                              ...
                              f"margin-top:-44px;margin-left:22px;"
                              f"box-shadow:0 2px 6px rgba(0,0,0,.4)'>{dn}</div>"),
                        icon_size=(180, 22)
                    )
                ).add_to(m)

        legend = (f"<div style='position:fixed;bottom:20px;left:20px;background:#0f2040;padding:10px 14px;"
                f"border-radius:8px;border:1px solid #1e3a6e;color:#e8f0fe;"
                f"font-family:sans-serif;font-size:11px;z-index:9999'>"
                f"<b>SSPI Risk</b><br>"
                ...
                f"<span style='color:{CLR['Low']}'>●</span> Low &lt;25</div>")
        m.get_root().html.add_child(folium.Element(legend))

        map_data = st_folium(
            m, width="100%", height=470,
            ...
            st.markdown(f"""<div class='ic {sal_class}'>
              <div style='display:flex;justify-content:space-between;align-items:flex-start'>
                ...
                <div style='text-align:right'>
                  <div style='font-size:.58rem;color:{T["text_sec"]};text-transform:uppercase;letter-spacing:.07em'>SSPI {latest_yr}</div>
                  <div class='big-score' style='color:{clr_sel}'>{latest_sspi:.1f}</div>
                  <div style='font-size:.78rem;color:{clr_sel};font-weight:700'>{sal_class} Risk</div>
                </div>
              </div>
              <div class='gauge' style='margin-top:.8rem'><div class='gfill' style='width:{bar_w}%;background:{clr_sel}'></div></div>
...
with tab2:
    ...
    with c3:
        ...
        ax3.axhline(dh["rainfall_annual"].mean(), color=T["accent"], ls="--", lw=1.2,
                    label=f"Mean {dh['rainfall_annual'].mean():.0f} mm")
        ax3.set_xlabel("Year")
    ...
    ax5.set_facecolor(T["chart_bg"])
    fig5.patch.set_facecolor(T["chart_fig"])
    ...
    for i, dn in enumerate(la_s["district"]):
        if dn == sel: 
            b5[i].set_edgecolor("white")
            b5[i].set_linewidth(2.5)
            b5[i].set_alpha(1.0)
    ...
    ax5.set_xticks(range(len(la_s)))
    ax5.set_xticklabels(la_s["district"], rotation=50, ha="right", fontsize=6.5, color=T["tick"])
    ax5.set_ylabel("SSPI Score", color=T["text_sec"])
    ax5.tick_params(colors=T["tick"])
...
with tab3:
    ...
    with cf1:
        ...
        ax6.set_facecolor(T["chart_bg"])
        fig6.patch.set_facecolor(T["chart_fig"])
        ...
        ax6.set_xlabel("Year", color=T["text_sec"])
        ax6.set_ylabel("SSPI Score", color=T["text_sec"])
    ...
    with cf2:
        ...
        for _, row in df.iterrows():
            yr = int(row["year"])
            v = row["predicted_sspi"]
            fc_cls = "Critical" if v >= 75 else "High" if v >= 50 else "Moderate" if v >= 25 else "Low"
            fc = sc(fc_cls)
    ...
    for i, (_, r) in enumerate(fore30.head(10).iterrows()):
        rc = sc("Critical" if r["predicted_sspi"] >= 75 else "High" if r["predicted_sspi"] >= 50 else "Moderate")
        with rc5[i % 5]:
...
with tab4:
    ...
    best_acc = round(best_r2 * 100, 1)
    avg_acc = round(metrics["r2"].mean() * 100, 1)
    worst_acc = round(metrics["r2"].min() * 100, 1)
    ...
    st.markdown("<div class='sh'>Model Cards — Detailed Performance</div>", unsafe_allow_html=True)
    ...
    for i, (_, row) in enumerate(metrics.iterrows()):
        ib = (i == best_idx)
        icon, clr_m = icons_m.get(row["model"], ("📊", T["accent"]))
    ...
    with ch1:
        ...
        for bar, val in zip(b7, metrics["r2"]):
            ax7.text(bar.get_width() + 0.8, bar.get_y() + bar.get_height() / 2, f"{val * 100:.1f}%",
                     va="center", color=T["text_pri"], fontsize=9, fontweight="bold")
        ax7.axvline(75, color=CLR["Low"], ls="--", lw=1.2, alpha=0.7, label="Target 75%")
        ax7.set_xlim(0, 110)
        ax7.set_xlabel("Accuracy % (R²)", color=T["text_sec"])
    ...
    with ch2:
        ...
        ax8.set_xticks(x)
        ax8.set_xticklabels(metrics["model"], rotation=15, ha="right", fontsize=7)
        ax8.set_ylabel("Error in SSPI units (lower = better)", color=T["text_sec"])
        ax8.set_title("MAE & RMSE Error Comparison", color=T["text_pri"], fontweight="bold")
    ...
    disp = metrics.copy()
    disp["accuracy_%"] = (disp["r2"] * 100).round(1)
    disp = disp.rename(columns=str.upper)
    fmt = {"MAE": "{:.2f}", "RMSE": "{:.2f}", "R2": "{:.3f}", "ACCURACY_%": "{:.1f}%"}
    if "MAPE" in disp.columns: 
        fmt["MAPE"] = "{:.1f}"
    ...
    st.markdown(f"""<div class='ic' style='margin-top:.8rem'>
      <div style='font-size:.6rem;color:{T["text_sec"]};text-transform:uppercase;letter-spacing:.08em;margin-bottom:.6rem'>SSPI Formula (Methodology Guide)</div>
      ...
      <div style='display:grid;grid-template-columns:1fr 1fr 1fr;gap:.5rem;margin-top:.7rem'>
        <div style='font-size:.73rem;color:{T["text_sec"]}'>🌿 <b style='color:{CLR["Low"]}'>NDVI deficit 40%</b><br>Vegetation stress — strongest salinity proxy</div>
        <div style='font-size:.73rem;color:{T["text_sec"]}'>🌧️ <b style='color:{T["accent"]}'>Rainfall deficit 35%</b><br>Low rain drives salt accumulation</div>
        <div style='font-size:.73rem;color:{T["text_sec"]}'>🌡️ <b style='color:{CLR["High"]}'>Temp anomaly 25%</b><br>High temp drives evaporation & capillary rise</div>
      </div></div>""", unsafe_allow_html=True)

    st.markdown("<div class='sh'>Transformer Components Used</div>", unsafe_allow_html=True)
    tf5 = st.columns(5)
    tf_items = [
        ("📥", "Input Embedding", "NDVI pixels → feature vectors", T["accent"]),
        ("📍", "Positional Encoding", "Year index + sspi_lag1", CLR["Low"]),
        ("🎯", "Attention Mechanism", "RF feature_importances_ = attention weights", CLR["High"]),
        ("🔄", "Autoregressive Decoding", "Rolling 2026–2030 forecast loop", CLR["Critical"]),
        ("🤖", "TFT Transformer", "Temporal Fusion Transformer Step 14", "#7c4dff")
    ]
    for col, (icon, name, desc, clr) in zip(tf5, tf_items):
        with col:
            st.markdown(f"<div class='mc'><div style='font-size:1.3rem;margin-bottom:.3rem'>{icon}</div>"
                        f"<div style='font-size:.7rem;font-weight:700;color:{clr};margin-bottom:.3rem'>{name}</div>"
                        f"<div style='font-size:.62rem;color:{T['text_sec']};line-height:1.4'>{desc}</div></div>",
                        unsafe_allow_html=True)
...
with tab5:
    ...
    st.markdown(
        f"<div style='font-size:.75rem;color:{T['accent']};font-weight:700;margin:.3rem 0 .6rem;padding:.3rem .8rem;"
        f"background:{T['card']};border:1px solid {T['accent']};border-radius:8px;display:inline-block'>"
        f"Showing: {LABELS[_c]}</div>", unsafe_allow_html=True)
    
    zf = st.multiselect("Zone filter", ["coastal", "canal", "inland", "hilly"],
                      default=["coastal", "canal", "inland", "hilly"], key="tab5_zone_filter")
...
st.markdown(f"<div class='footer'>🌾 Gujarat Soil Salinity Intelligence Platform · ML + Temporal Fusion Transformer · 33 Districts · 2015–2030 · MODIS GEE · NASA POWER · CSSRI Bharuch</div>", unsafe_allow_html=True)
