...
# ── HARDCODED ZONE MAP — source of truth, never depends on DB ─────────────────
# This guarantees hilly districts always show correctly regardless of DB content
ZONE_MASTER = {
    # Coastal
    "Kachchh":         "coastal",
    "Jamnagar":        "coastal",
    "DevbhumiDwarka":  "coastal",
    "Junagadh":        "coastal",
    "GirSomnath":      "coastal",
    "Bhavnagar":       "coastal",
    "Amreli":          "coastal",
...
def load_data():
    ...
    if not os.path.exists(db):
        st.error("❌ Database not found. Run all scripts first.")
        st.stop()
    ...
    except: m = DEFAULT_METRICS.copy()
    conn.close()

    # ── Always recompute sspi_class from score ────────────────────────────────
    h["sspi_class"] = h["sspi"].apply(compute_sspi_class)

    # ── Overwrite zone_type from DB with ZONE_MASTER — guarantees hilly shows ─
    # DB may have trailing spaces, wrong case, or missing values for hilly dists
    h["zone_type"] = h["district"].map(ZONE_MASTER).fillna("inland")
    f["zone_type"] = f["district"].map(ZONE_MASTER).fillna("inland") if "district" in f.columns else f.get("zone_type", "inland")
...
with st.sidebar:
    ...
    for col,(icon,zone) in zip(qz_cols,ZONE_LIST):
        ...
        with col:
            ...
            if clicked:
                # Use zone_map (backed by ZONE_MASTER) — hilly always has 7 districts
...
# Always get zone_type from ZONE_MASTER — never trust DB column
...
with tab1:
    ...
    with cm:
        ...
        # Filter map rows by active zone (from ZONE_MASTER, not DB)
        if st.session_state.active_zone:
            map_rows = latest_all[
        ...
        for _,row in map_rows.iterrows():
            ...
            clr       = sc(dot_class)
            # Zone badge for tooltip from ZONE_MASTER
...
with tab2:
    ...
    # Use ZONE_MASTER for chart coloring too
...
with tab4:
    ...
    tf5=st.columns(5)
    tf_items=[("📥","Input Embedding","NDVI pixels → feature vectors",T["accent"]),
